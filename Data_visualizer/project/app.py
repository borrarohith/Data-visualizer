...
def display_excel_info():
    ...
    file_path = "C:\\Users\\vinay\\vs code\\kiet.csv"
    ...
    unique_columns = df.columns[df.nunique() == df.shape[0]].tolist()  # Convert unique_columns to a list
    ...
    insights = {
        ...
        "Mean": dict(df.mean(numeric_only=True)),
        "Mode": dict(df.mode().iloc[0]),
        "Median": dict(df.median(numeric_only=True)),
        "Range": dict(df.max(numeric_only=True) - df.min(numeric_only=True)),
    }
    ...
    return render_template('graphs.html', df_shape=(num_rows, num_columns),insights=insights, selected_graphs=selected_graphs, top_5_rows=top_5_rows, categorized_columns=categorized_columns)
